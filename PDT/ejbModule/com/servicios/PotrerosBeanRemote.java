...
//			potrero.setDesde(new Date());	//Los desde los seteo todos a la vez al final
		...
		throw new PotrerosException("ERROR", "ERROR DE ALTA", "El id " + potrero.getId() + " no es válido");
	}
...
//				potreroEditado.setId(null);	//al final habrá q recorrer los null para hacerle un persist
//				this.listaNuevas.put(potreroEditado.getId(), potreroEditado);
//			}
			//Se supone que la forma sigue igual, así que simplemente remplazo la existente con la editada mergeado
			this.em.merge(potreroEditado);
			this.listaExistentes.put(potreroEditado.getId(), potreroEditado);
		}
	...
	public boolean guardarTodo() throws DatosInvalidosException {
		...
		for (Potrero potreroADesactivar : this.listaDesactivar.values()) {
			...
			System.out.println("Potrero borrada quedó: " + potrero.getNombre() + ", " + potrero.getActivo() + ", " + potrero.getHasta().toString());
		}
		...
		try {
			...
			throw new DatosInvalidosException("Hubo un problema al persistir una zona borrada", e);
		}
...
//				this.em.persist(zona);	//Ya tiene seteado el desde
		...
		try {
			...
			this.em.getTransaction().rollback();
			throw new DatosInvalidosException("Hubo un problema al persistir una nueva zona", e);
		}
	...
	private void desactivarZonaPotreros(Potrero potreroDesactivado) {
		...
		System.out.println("Hay " + listaZonaPotreroActivas.size() + " ZonaPotrero activas en el potrero " + potreroDesactivado.getNombre());
